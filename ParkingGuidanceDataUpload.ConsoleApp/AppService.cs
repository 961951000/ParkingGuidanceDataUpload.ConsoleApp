            ...
            while (true)
            {
                while (true)
                {
                    try
                    {
                        ...
                        var testInfo = await _db.QuerySingleAsync("SELECT CONVERT (VARCHAR(20), 3401110130) as [ParkId],1192 as [CountCw],800 as[PrepCw] FROM [dbo].[Tc_ParkingLotInfo] WHERE ParkingLotName = 'B2'");
